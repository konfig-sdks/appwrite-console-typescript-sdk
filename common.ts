...
import { readableStreamToString, AppwriteConsoleError, parseIfJson } from "./error";
        ...
        if (e instanceof AxiosError && e.isAxiosError) {
            try {
                const responseBody =
                    ...
                    : e.response?.data
                throw new AppwriteConsoleError(e, parseIfJson(responseBody))
            } catch (innerError) {
                if (innerError instanceof ReferenceError) {
                    ...
                    throw new AppwriteConsoleError(e, e.response?.data)
                }
                if (innerError instanceof AppwriteConsoleError) {
                    // Got "AppwriteConsoleError" from the above try block
                    throw innerError;
                }
